...
//
//   Hero → 놀이 선정 이유(핑크) → 이달의 놀이 흐름(블루, Week1~4) → 교사의 기대(그린) → 주요 놀이 요소(퍼플)
//
// 섹션 배경=연한 톤, 콘텐츠(배지/번호/제목)=진한 색 대비. 제목=ONE Mobile POP, 본문=SUIT.
...
export function buildMonthlyPlanBlueprint(recipe: DesignRecipe): TemplateBlueprint {
  ...
  const sectionCard = (fill: string): ShapeStyle => ({ fill, radius: 32, opacity: 1 });

  // 타이포 토큰
  const heroTitleStyle: TextStyle = { fontSize: 96, weight: 800, color: T.hero.title, align: "center", fontFamily: TITLE_FONT };
  ...
  // ── shape (카드 / 배지) ── 섹션 배경=연한 톤, 배지=진한 강조
  const shapes: ShapeElement[] = [
    shape("HeroCard", "rect", { x: 60, y: 60, width: 960, height: 360 }, { fill: T.hero.bg, radius: 40 }, 1),
    shape("AgeBadge", "pill", { x: 330, y: 296, width: 220, height: 64 }, pill(T.hero.accent), 2),
    shape("ThemeBadge", "pill", { x: 560, y: 296, width: 300, height: 64 }, pill(T.hero.accent), 2),
    shape("ReasonCard", "rect", { x: 60, y: 460, width: 960, height: 230 }, sectionCard(S.reason.bg), 1),
    ...
    shape("ExpectationCard", "rect", { x: 60, y: 1350, width: 960, height: 250 }, sectionCard(S.expectation.bg), 1),
    shape("ExpectationBadge", "pill", { x: 350, y: 1366, width: 380, height: 70 }, pill(S.expectation.accent), 2),
    shape("ElementCard", "rect", { x: 60, y: 1630, width: 960, height: 230 }, sectionCard(S.element.bg), 1),
    shape("ElementBadge", "pill", { x: 350, y: 1646, width: 380, height: 66 }, pill(S.element.accent), 2),
  ];
  ...
  const texts: TextElement[] = [
    text("Title", "hero", C.title, { x: 100, y: 110, width: 880, height: 140 }, heroTitleStyle, 5, "meta.title"),
    text("Age", "hero_age", C.age, { x: 330, y: 312, width: 220, height: 36 }, badgeChip, 6, "basic_info.age"),
    text("Theme", "hero_theme", C.theme, { x: 560, y: 312, width: 300, height: 36 }, badgeChip, 6, "basic_info.theme"),
    ...
    text("ExpectationTitle", "teacher_expectation", C.expectationTitle, { x: 350, y: 1380, width: 380, height: 42 }, badgeTitle, 7, "sections.teacher_expectation"),
    text("ElementTitle", "learning_elements", C.elementTitle, { x: 350, y: 1660, width: 380, height: 40 }, badgeTitle, 7, "sections.learning_elements"),
  ];
  weekPos.forEach((p, i) => {
    const wc = T.weeks[i];
    texts.push(text(`Week${i + 1}Num`, `week_${i + 1}_num`, `${i + 1}주`, { x: p.x + 24, y: p.y + 36, width: 64, height: 36 }, weekNumStyle, 8));
    texts.push(text(`Week${i + 1}Title`, `week_${i + 1}`, C.weeks[i] ?? DEFAULT_CONTENT.weeks[i], { x: p.x + 100, y: p.y + 30, width: 340, height: 48 }, { fontSize: 40, weight: 800, color: wc.accent, align: "left", fontFamily: TITLE_FONT }, 8, `weekly_flow[${i}].title`));
  });
